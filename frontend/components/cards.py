...
def render_kpi_card(title: str, value: str | int, subtitle: str, icon: str, theme: str = "blue"):
    ...
    card_html = (
        ...
        'box-shadow:0 4px 15px rgba(0,0,0,0.25);min-height:144px;height:144px;box-sizing:border-box;'
        'transition:transform 0.2s ease, box-shadow 0.2s ease;">'
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">'
        '<div style="min-width:0;flex:1 1 130px;">'
        '<p style="color:#e2e8f0;font-size:0.76rem;font-weight:700;text-transform:uppercase;'
        'letter-spacing:0.5px;line-height:1.25;min-height:38px;margin:0 0 4px 0;overflow-wrap:anywhere;">{title}</p>'
        ...
        'display:flex;align-items:center;justify-content:center;line-height:1;flex:0 0 46px;'
        'box-shadow:0 2px 8px rgba(0,0,0,0.15);"><span class="material-symbols-rounded" style="font-size:1.5rem;">{icon}</span></div>'
        '</div></div>'
    ).format(
...
def render_parking_summary_card(parking: Dict[str, Any]):
    ...
    if parking.get("distance_km") is not None:
        distance_tag = f"""<span style="background: rgba(59, 130, 246, 0.25); color: #93c5fd; border: 1px solid rgba(59, 130, 246, 0.5); padding: 4px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 700; margin-right: 6px;"><span class="material-symbols-rounded">location_on</span> {parking['distance_km']} km away</span>"""

    ev_tag = ""
    if parking.get("ev_available"):
        ev_tag = """<span style="background: rgba(16, 185, 129, 0.25); color: #6ee7b7; border: 1px solid rgba(16, 185, 129, 0.5); padding: 4px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 700; margin-right: 6px;"><span class="material-symbols-rounded">ev_station</span> EV Charging</span>"""

    acc_tag = ""
    if parking.get("accessible_available"):
        acc_tag = """<span style="background: rgba(139, 92, 246, 0.25); color: #c4b5fd; border: 1px solid rgba(139, 92, 246, 0.5); padding: 4px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 700;"><span class="material-symbols-rounded">accessible</span> Accessible</span>"""
...
def get_slot_status_badge(status: str) -> str:
    ...
    styles = {
        "available": ("#34d399", "rgba(16, 185, 129, 0.25)", "<span class='material-symbols-rounded'>check_circle</span> Available"),
        "reserved": ("#fbbf24", "rgba(245, 158, 11, 0.25)", "<span class='material-symbols-rounded'>event</span> Reserved"),
        "occupied": ("#f87171", "rgba(239, 68, 68, 0.25)", "<span class='material-symbols-rounded'>directions_car</span> Occupied"),
        "maintenance": ("#94a3b8", "rgba(100, 116, 139, 0.25)", "<span class='material-symbols-rounded'>build</span> Maintenance")
    }
