...
def render_parking_management():
    ...
    header_html = (
        ...
        '<h1 style="color:#ffffff;font-size:1.85rem;font-weight:800;margin:0;"><span class="material-symbols-rounded">local_parking</span> Parking Facility Management</h1>'
    ...
    location_tab, slots_tab, sessions_tab = st.tabs([":material/apartment: Facilities & Locations", ":material/local_parking: Slot Operations", ":material/directions_car: Live Sessions"])
    ...
    with location_tab:
        ...
        with st.expander(":material/add_business: Add New Parking Location", expanded=(len(locations) == 0)):
            with st.form("add_parking_location"):
                ...
                if st.form_submit_button(":material/add_circle: Register Location", type="primary", use_container_width=True):
        ...
        for location in locations:
            ...
            with st.expander(f"{status_badge_icon} {location['parking_name']} ({location.get('area')})  {status_text}  {fee_val:.2f}/hr"):
                ...
                with st.form(f"edit_location_{location['parking_id']}"):
                    ...
                    if st.form_submit_button(":material/save: Save Details", type="primary", use_container_width=True):
    ...
    with slots_tab:
        ...
        if not locations:
            ...
            with st.expander(":material/add_circle: Add New Slot to Facility"):
                with st.form("add_slot"):
                    ...
                    if st.form_submit_button(":material/add_circle: Add Slot", type="primary", use_container_width=True):
                        if not number.strip():
                ...
                if slots:
                    ...
                    for index, slot in enumerate(slots):
                        ...
                        with cols[index % 4]:
                            ...
                            if st.button(":material/sync: Update Status", key=f"update_slot_{slot['slot_id']}", use_container_width=True):
